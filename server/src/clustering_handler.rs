...
fn compare_next_pair(module: &mut BusinessProcessAnalysisModule, clustering_attempt: &mut Individual) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if indices[0] >= processes.len() || indices[1] >= processes.len() {
        ...
        update_individual(module, clustering_attempt)?;
        info!("Successfully completed comparing all pairs for attempt: {}", clustering_attempt.get_id());
        return Ok(());
    }
    ...
    info!("Comparison result for processes {} and {}: {}", processes[indices[0]], processes[indices[1]], if is_similar { "similar" } else { "different" });
...
fn build_clusters(module: &mut BusinessProcessAnalysisModule, clustering_attempt: &mut Individual) -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("Adding {} isolated processes to the graph", all_processes.len());
    ...
    info!("Found {} distinct clusters", clusters.len());
    ...
    info!("Setting clustering status to Completed");
